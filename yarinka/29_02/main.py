...
def web_app(env, start_response):
    ...
    if path == "/":
        ...
        info = form.getfirst("info", "")
        if name != "" and phone != "":
            html_content = FriendPhones().new_friend(name, phone, info)
            text = text.replace("result", html_content)
        ...
        phone = form.getfirst("last_name1", "")
        ...
        if name != "" and phone == "":
            html_content = " "
            for i in FriendPhones().find_friend(name, phone):
                html_content += i + "  ,"
            text = text.replace("result", html_content)
            start_response("200 OK", [("Content-type:", "text/html;  cherset=utf-8")])
            return [bytes(text, encoding="utf-8")]
        ...
        phone = form.getfirst("last_name", "")
        if name != "" and phone != "":
            text = text.replace("result", FriendPhones().delete_friend(name, phone))
            start_response("200 OK", [("Content-type:", "text/html;  cherset=utf-8")])
            return [bytes(text, encoding="utf-8")]
        else:
            start_response("200 OK", [("Content-type:", "text/html;  cherset=utf-8")])
