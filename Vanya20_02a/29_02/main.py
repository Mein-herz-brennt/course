...
def web_app(env, start_response):
    ...
    if path == "/":
        ...
        if name != "" and phone != "":
            html_content = FriendBook().new_friend(name, phone)
        ...
        if new_name != "" and phone != "":
            html_content = FriendBook().change_friend_name(new_name, phone)
        ...
        if name != "" and new_phone != "":
            html_content = FriendBook().change_friend_phone(name, new_phone)
            text = text.replace("Виправимо!", html_content)
            start_response("200 OK", [("Content-type:", "text/html;  cherset=utf-8")])
            return [bytes(text, encoding="utf-8")]
        else:
        ...
        phone = form.getfirst("last_name", "")
        if name != "" and phone != "":
            text = text.replace("result", FriendBook().delete_friend(name, phone))
            start_response("200 OK", [("Content-type:", "text/html;  cherset=utf-8")])
            return [bytes(text, encoding="utf-8")]
        else:
            start_response("200 OK", [("Content-type:", "text/html;  cherset=utf-8")])
        ...
        if name != "" and phone == "":
            html_content = FriendBook().find_friend(name, phone)
