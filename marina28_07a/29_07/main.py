...
def web_app(env, start_response):
    ...
    if path == "/":
        ...
        with open("update.html", "r", encoding="utf-8") as file:
            text = file.read()
        cur = ""
        for i in Driver().get_currencies():
            cur += OPTION.replace("cur", i)
        text = text.replace("currencies", cur)
        name = form.getfirst("pib", "")
        date_now = form.getfirst("date", "")
        new_km = form.getfirst("km", "")
        if name != "" and date_now != "" and new_km != "":
            text = text.replace("result", Driver().update_waybill(name, date_now, new_km))
            start_response("200 OK", [("Content-type:", "text/html;  cherset=utf-8")])
            return [bytes(text, encoding="utf-8")]
        else:
            start_response("200 OK", [("Content-type:", "text/html;  cherset=utf-8")])
        ...
        name = form.getfirst("pib", "")
        date_now = form.getfirst("date", "")
        if name != "" and date_now != "":
            text = text.replace("result", Driver().delete_waybill(name, date_now))
            start_response("200 OK", [("Content-type:", "text/html;  cherset=utf-8")])
            return [bytes(text, encoding="utf-8")]
        else:
            start_response("200 OK", [("Content-type:", "text/html;  cherset=utf-8")])
