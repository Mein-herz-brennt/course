...
def web_app(env, start_response):
    ...
    if path == "/":
        ...
        if name == "":
            ...
            with open(toys.result_xml(toys.find_toy_by_name(name)), "r", encoding="utf-8") as file:
                text = file.read()
            start_response("200 OK", [("Content-type:", "text/xml;  cherset=utf-8")])
        ...
        if age == "":
            ...
            with open(toys.result_xml(toys.find_toys_by_years(years=int(age))), "r", encoding="utf-8") as file:
                text = file.read()
                start_response("200 OK", [("Content-type:", "text/xml;  cherset=utf-8")])
                return [bytes(text, encoding="utf-8")]
        ...
        if price_min == "" and price_max == "":
            ...
            with open(
                    toys.result_xml(toys.find_by_min_and_max_price(min_price=int(price_min), max_price=int(price_max))),
